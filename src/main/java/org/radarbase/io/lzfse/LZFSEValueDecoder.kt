...
constructor(nStates: Int) {
    ...
    fun load(
        weights: ShortArray,
        symbolVBits: ByteArray,
        symbolVBase: IntArray,
        state: Int,
    ) {
        tans.apply {
            init(weights)
            table.forEach { v -> v[symbolVBits] = symbolVBase }
        }
        this.state.value = state
    }

    fun decode(`in`: BitInStream): Int = tans
        .transition(state, `in`)
        .readVData(`in`)

    override fun toString(): String = "LZFSEValueDecoder{tans=$tans, state=$state}"
    ...
    internal class ValueEntry : TANS.Entry() {
        ...
        operator fun set(symbolVBits: ByteArray, symbolVBase: IntArray) {
